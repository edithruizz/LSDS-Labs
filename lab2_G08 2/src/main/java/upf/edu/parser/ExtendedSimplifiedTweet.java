...
// IMPLEMENT ME
		...
		try {
		  ...
		  long followersCount =userObj.get("followers_count")
				  .getAsLong() ;
		  ...
		  try {
			  ...
			  JsonObject isRetweetedObj =jo.get("retweeted_status")
					  .getAsJsonObject();
					 JsonObject retweetedUserObject=isRetweetedObj.get("user").getAsJsonObject();
					  retweetedUserId = retweetedUserObject.get("id")
							   .getAsLong();
					  retweetedTweetId= isRetweetedObj.get("id")
							   .getAsLong();
					  isRetweeted = true;
